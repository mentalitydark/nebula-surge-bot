...
describe('CreateBuildUseCase - Testes Unitários', () => {
  ...
  it('Deve criar uma build', async () => {
    ...
    const build = await useCase.execute({ equipament: 'equipament', content: 'content' })

    assert.ok(build.id)
    assert.strictEqual(build.equipament, 'equipament')
  ...
  it('Deve retornar um erro quando o equipamento já existir', async () => {
    ...
    await assert.rejects(useCase.execute({ equipament: 'equipament', content: 'content' }), ConflictError)
  })
})